...
public class RestHttpDispatcher extends AbstractRemoteDispatcher implements RemoteDispatcher {
    ...
    @Override
    protected TextDispatcherResult executeRemote(final Command<? extends Result> command, final TextDispatcherCommand textDispatcherCommand) throws DispatchException {
	try {
	    HttpRequestBase httpRequest = this.createRequest(command, textDispatcherCommand);
